...
public class Setmine implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (!p.hasPermission("Tom.setmine")) {
			...
			if (args[0].equalsIgnoreCase("1")) {
				...
				config10.set("10.World", p.getWorld().getName());
				config10.set("10.X", p.getLocation().getX());
				config10.set("10.Y", p.getLocation().getY());
				config10.set("10.Z", p.getLocation().getZ());
				config10.set("10.Yaw", p.getLocation().getYaw());
				config10.set("10.Pitch", p.getLocation().getPitch());
				Main.getPlugin().saveConfig();
				p.sendMessage(Main.pre + "§3Du hast Mine 10 gesetzt!");

			} else if (args[0].equalsIgnoreCase("event")) {
				FileConfiguration config10 = Main.getPlugin().getConfig();
				config10.set("event.World", p.getWorld().getName());
				config10.set("event.X", p.getLocation().getX());
				config10.set("event.Y", p.getLocation().getY());
				config10.set("event.Z", p.getLocation().getZ());
				config10.set("event.Yaw", p.getLocation().getYaw());
				config10.set("event.Pitch", p.getLocation().getPitch());
				Main.getPlugin().saveConfig();
				p.sendMessage(Main.pre + "§3Du hast die Event Mine gesetzt!");

			} else {
				p.sendMessage(Main.pre + "§cBitte benutze §6/setmine <Mine>§c!");
			}
