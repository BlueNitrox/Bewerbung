...
public class Chest_ManagerMaster {

	private final Main plugin;
	private BukkitTask task;

	public Chest_ManagerMaster(Main plugin) {
		this.plugin = plugin;
	...
	public List<ItemStack> getItem() {
		...
		return cl.getMasterChestContent();
	}
	...
	public void startAnimation(Player p) {
		...
		Inventory inv = Bukkit.getServer().createInventory(null, 27, "§dMaster-Chest");
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				if (p.getOpenInventory() == null) {
					...
					p.sendMessage(Main.pre + "§aDu hast §e" + i.getAmount() + "x " + i.getItemMeta().getDisplayName()
							+ " §agewonnen");
					...
					FireworkEffect effect = FireworkEffect.builder().withColor(Color.RED).flicker(true).trail(true)
							.withFade(Color.AQUA).with(FireworkEffect.Type.BALL_LARGE).build();
				...
				if (sleep == 1000) {
					...
					p.sendMessage(Main.pre + "§aDu hast §e" + inv.getItem(12).getAmount() + "x "
							+ inv.getItem(12).getItemMeta().getDisplayName() + " §agewonnen");
					p.closeInventory();
					p.playSound(p.getLocation(), Sound.NOTE_PLING, 1L, 1L);
