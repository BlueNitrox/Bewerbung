...
public class Minenreset8 implements CommandExecutor {


	@Override
	public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args) {
		Player p = (Player)cs;
		if(p.hasPermission("minenreset.8")) {
			
			Location loc11 = new Location(Bukkit.getWorld("world"), 115, 82, -110);
			...
			Location l = new Location(Bukkit.getWorld("world"), 118, 82, -108);
			int x = l.getBlockX();
			int y = l.getBlockY();
			int z = l.getBlockZ();
			World w = l.getWorld();
			...
			Location l2 = new Location(Bukkit.getWorld("world"), 135, 64, -126);
			int x2 = l2.getBlockX();
			int y2 = l2.getBlockY();
			int z2 = l2.getBlockZ();
				      ...
				      for (Player all: Bukkit.getOnlinePlayers()) {
			    		  if(all.getLocation().getBlockX() >= 118 && all.getLocation().getBlockX() <= 135 ) {
			    			  if(all.getLocation().getBlockY() >=64 && all.getLocation().getBlockY() <= 82) {
			    				  if(all.getLocation().getBlockZ() >=-126  && all.getLocation().getBlockZ() <= -108 ) {
			    					  all.teleport(loc11);
			    				  }
