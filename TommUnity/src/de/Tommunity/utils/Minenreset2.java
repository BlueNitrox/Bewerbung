...
public class Minenreset2 implements CommandExecutor {
	...
	public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args) {
		...
		if(p.hasPermission("minenreset.2")) {
			Location loc11 = new Location(Bukkit.getWorld("world"), 6, 79, -30);
			...
			Location l = new Location(Bukkit.getWorld("world"), 15, 78, -25);
			int x1 = l.getBlockX();
			int y1 = l.getBlockY();
			int z1 = l.getBlockZ();
			World w = l.getWorld();
			...
			Location l2 = new Location(Bukkit.getWorld("world"), -3, 63, -7);
			int x2 = l2.getBlockX();
			int y2 = l2.getBlockY();
			int z2 = l2.getBlockZ();
				  ...
				  for(int y3 = minY; y3 <= maxY; y3++){
				    for(int z3 = minZ; z3 <= maxZ; z3++){
				      ...
				      for (Player all: Bukkit.getOnlinePlayers()) {
				    		  if(all.getLocation().getBlockX() >= -3 && all.getLocation().getBlockX() <= 15 ) {
				    			  if(all.getLocation().getBlockY() >=1  && all.getLocation().getBlockY() <= 78 ) {
				    				  if(all.getLocation().getBlockZ() >=-25  && all.getLocation().getBlockZ() <= -7 ) {
				    					  all.teleport(loc11);
				    				  }
