...
public class Minenreset9 implements CommandExecutor {


	@Override
	public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args) {
		Player p = (Player)cs;
		if(p.hasPermission("minenreset.9")) {
			
			Location loc11 = new Location(Bukkit.getWorld("world"), 114, 83, -175);
			...
			Location l = new Location(Bukkit.getWorld("world"), 118, 82, -166);
			int x = l.getBlockX();
			int y = l.getBlockY();
			int z = l.getBlockZ();
			World w = l.getWorld();
			...
			Location l2 = new Location(Bukkit.getWorld("world"), 135, 64, -184);
			int x2 = l2.getBlockX();
			int y2 = l2.getBlockY();
			int z2 = l2.getBlockZ();
				  ...
				  for(int y3 = minY; y3 <= maxY; y3++){
				    for(int z3 = minZ; z3 <= maxZ; z3++){
				      ...
				      for (Player all: Bukkit.getOnlinePlayers()) {
			    		  if(all.getLocation().getBlockX() >= 118 && all.getLocation().getBlockX() <= 135 ) {
			    			  if(all.getLocation().getBlockY() >=64 && all.getLocation().getBlockY() <= 82) {
			    				  if(all.getLocation().getBlockZ() >=-184  && all.getLocation().getBlockZ() <= -166 ) {
			    					  all.teleport(loc11);
			    				  }
