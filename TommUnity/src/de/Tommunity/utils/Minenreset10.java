...
public class Minenreset10 implements CommandExecutor {
	...
	public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args) {
		Player p = (Player)cs;
		if(p.hasPermission("minenreset.10")) {
			
			Location loc11 = new Location(Bukkit.getWorld("world"), 85, 92, -163);
			...
			Location l = new Location(Bukkit.getWorld("world"), 81, 91, -170);
			int x = l.getBlockX();
			int y = l.getBlockY();
			int z = l.getBlockZ();
			World w = l.getWorld();
			...
			Location l2 = new Location(Bukkit.getWorld("world"), 64, 73, -152);
			int x2 = l2.getBlockX();
			int y2 = l2.getBlockY();
			int z2 = l2.getBlockZ();
				  ...
				  for(int y3 = minY; y3 <= maxY; y3++){
				    for(int z3 = minZ; z3 <= maxZ; z3++){
				      ...
				      for (Player all: Bukkit.getOnlinePlayers()) {
			    		  if(all.getLocation().getBlockX() >= 64 && all.getLocation().getBlockX() <= 81 ) {
			    			  if(all.getLocation().getBlockY() >=73 && all.getLocation().getBlockY() <= 91) {
			    				  if(all.getLocation().getBlockZ() >= -170  && all.getLocation().getBlockZ() <= -152 ) {
			    					  all.teleport(loc11);
			    				  }
