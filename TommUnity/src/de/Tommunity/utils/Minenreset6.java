...
public class Minenreset6 implements CommandExecutor {


	@Override
	public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args) {
		Player p = (Player)cs;
		if(p.hasPermission("minenreset.6")) {
			
			Location loc11 = new Location(Bukkit.getWorld("world"), 79, 78, -30);
			...
			Location l = new Location(Bukkit.getWorld("world"), 82, 77, -22);
			int x = l.getBlockX();
			int y = l.getBlockY();
			int z = l.getBlockZ();
			World w = l.getWorld();
			...
			Location l2 = new Location(Bukkit.getWorld("world"), 100, 62, -40);
			int x2 = l2.getBlockX();
			int y2 = l2.getBlockY();
			int z2 = l2.getBlockZ();
				  ...
				  for(int y3 = minY; y3 <= maxY; y3++){
				    for(int z3 = minZ; z3 <= maxZ; z3++){
				      ...
				      for (Player all: Bukkit.getOnlinePlayers()) {
			    		  if(all.getLocation().getBlockX() >= 82 && all.getLocation().getBlockX() <= 100 ) {
			    			  if(all.getLocation().getBlockY() >=1 && all.getLocation().getBlockY() <= 77 ) {
			    				  if(all.getLocation().getBlockZ() >=-40  && all.getLocation().getBlockZ() <= -22 ) {
			    					  all.teleport(loc11);
			    				  }
