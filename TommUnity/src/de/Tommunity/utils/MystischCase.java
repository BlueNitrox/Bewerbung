...
public class MystischCase {

	private final Main plugin;
	private BukkitTask task;
	
	public MystischCase(Main plugin) {
		this.plugin = plugin;
		
	}

	public List<ItemStack> getItem(){
		MystischCaseItem my = new MystischCaseItem(plugin);
		return my.getMystCaseContent();
	}
	public void startAnimation(Player p) {
		...
		Inventory inv = Bukkit.getServer().createInventory(null, 27, "§9§lMystische-Case");
	...
	public void onklick(InventoryClickEvent e) {
		if(e.getClickedInventory().getTitle().equals("§9§lMystische-Case")) {
			e.setCancelled(true);
		}
	}
