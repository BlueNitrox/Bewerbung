...
public class Minenreset5 implements CommandExecutor {


	@Override
	public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args) {
		Player p = (Player)cs;
		if(p.hasPermission("minenreset.5")) {
			
			Location loc11 = new Location(Bukkit.getWorld("world"), -26, 57, 17);
			...
			Location l = new Location(Bukkit.getWorld("world"), -38, 56, 39);
			int x = l.getBlockX();
			int y = l.getBlockY();
			int z = l.getBlockZ();
			World w = l.getWorld();
			...
			Location l2 = new Location(Bukkit.getWorld("world"), -16, 39, 20);
			int x2 = l2.getBlockX();
			int y2 = l2.getBlockY();
			int z2 = l2.getBlockZ();
				  ...
				  for(int y3 = minY; y3 <= maxY; y3++){
				    for(int z3 = minZ; z3 <= maxZ; z3++){
				      ...
				      for (Player all: Bukkit.getOnlinePlayers()) {
			    		  if(all.getLocation().getBlockX() >= -38 && all.getLocation().getBlockX() <= -16 ) {
			    			  if(all.getLocation().getBlockY() >=39 && all.getLocation().getBlockY() <= 56 ) {
			    				  if(all.getLocation().getBlockZ() >= 20  && all.getLocation().getBlockZ() <= 39 ) {
			    					  all.teleport(loc11);
			    				  }
