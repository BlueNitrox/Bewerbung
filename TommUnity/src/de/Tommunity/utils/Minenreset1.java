...
public class Minenreset1 implements CommandExecutor {
	...
	public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args) {
		...
		if(p.hasPermission("minenreset.1")) {
			
			Location loc11 = new Location(Bukkit.getWorld("world"), 7, 81, -42);
			...
			Location l = new Location(Bukkit.getWorld("world"), -2, 80, -49);
			int x1 = l.getBlockX();
			int y1 = l.getBlockY();
			int z1 = l.getBlockZ();
			World w = l.getWorld();
			...
			Location l2 = new Location(Bukkit.getWorld("world"), 16, 65, -67);
			int x2 = l2.getBlockX();
			int y2 = l2.getBlockY();
			int z2 = l2.getBlockZ();
				  ...
				  for(int y3 = minY; y3 <= maxY; y3++){
				    for(int z3 = minZ; z3 <= maxZ; z3++){
				      ...
				      for (Player all: Bukkit.getOnlinePlayers()) {
			    		  if(all.getLocation().getBlockX() >= -2 && all.getLocation().getBlockX() <= 16 ) {
			    			  if(all.getLocation().getBlockY() >=65  && all.getLocation().getBlockY() <= 80 ) {
			    				  if(all.getLocation().getBlockZ() >=-67  && all.getLocation().getBlockZ() <= -47 ) {
			    					  all.teleport(loc11);
			    				  }
