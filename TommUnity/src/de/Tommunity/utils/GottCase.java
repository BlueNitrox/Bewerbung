...
public class GottCase {

	private final Main plugin;
	private BukkitTask task;
	
	public GottCase(Main plugin) {
		this.plugin = plugin;
		
	}

	public List<ItemStack> getItem(){
		GottCaseItem my = new GottCaseItem(plugin);
		return my.getGottCaseContent();
	}
	public void startAnimation(Player p) {
		...
		Inventory inv = Bukkit.getServer().createInventory(null, 27, "§c§lGöttliche-Case");
	...
	public void onklick(InventoryClickEvent e) {
		if(e.getClickedInventory().getTitle().equals("§c§lGöttliche-Case")) {
			e.setCancelled(true);
		}
	}
