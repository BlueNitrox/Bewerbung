...
public class Minenreset7 implements CommandExecutor {


	@Override
	public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args) {
		Player p = (Player)cs;
		if(p.hasPermission("minenreset.7")) {
			
			Location loc11 = new Location(Bukkit.getWorld("world"), 99, 80, -66);
			...
			Location l = new Location(Bukkit.getWorld("world"), 102, 79, -66);
			int x = l.getBlockX();
			int y = l.getBlockY();
			int z = l.getBlockZ();
			World w = l.getWorld();
			...
			Location l2 = new Location(Bukkit.getWorld("world"), 119, 61, -84);
			int x2 = l2.getBlockX();
			int y2 = l2.getBlockY();
			int z2 = l2.getBlockZ();
				  ...
				  for(int y3 = minY; y3 <= maxY; y3++){
				    for(int z3 = minZ; z3 <= maxZ; z3++){
				      ...
				      for (Player all: Bukkit.getOnlinePlayers()) {
			    		  if(all.getLocation().getBlockX() >= 102 && all.getLocation().getBlockX() <= 119 ) {
			    			  if(all.getLocation().getBlockY() >=61 && all.getLocation().getBlockY() <= 79) {
			    				  if(all.getLocation().getBlockZ() >=-84  && all.getLocation().getBlockZ() <= -66 ) {
			    					  all.teleport(loc11);
			    				  }
